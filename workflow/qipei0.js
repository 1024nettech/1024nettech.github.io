...
import * as publics from "./public0.js"
...
export async function openProductsEdit() {
    ...
    // 使用 .map() 方法提取 id，并确保顺序
    let productIds = productLinks.map(function () {
        let url = $(this).attr("href");
        let idMatch = url.match(/[?&]id=(\d+)/);
        return idMatch ? idMatch[1] : null; // 提取 id 或返回 null
    }).get(); // 使用 .get() 转换为数组
...
export async function handleProductAction(checked_car, status = "") {
    ...
    if (status === "未处理") {
        ...
        await publics.appendToRecord(username, ch_id, id, record);
        window.close();
    } else {
